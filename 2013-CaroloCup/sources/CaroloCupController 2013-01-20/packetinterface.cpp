...
bool PacketInterface::connectUdp(QHostAddress addr, quint16 port)
{
    mPort = port;
    mHostAddress = addr;
    return mUdpSocket->bind(QHostAddress::Any, mPort + 1);
}
...
void PacketInterface::processPacket(const unsigned char *data, int len)
{
    ...
    case CAR_PACKET_READ_SENS_IR:
        ...
        for (int i = 0;i < tmp_us;i++) {
            ir_values.append(((quint16)data[i * 2] << 8) |
                             (quint16)data[i * 2 + 1]);
        }
        emit carSensorsIrReceived(ir_values);
        break;
...
bool PacketInterface::readSensorsIr()
{
    QByteArray buffer;
    buffer.clear();
    buffer.append((char)CAR_PACKET_RES);
    buffer.append((char)CAR_PACKET_READ_SENS_IR);
    return sendPacket(buffer);
}

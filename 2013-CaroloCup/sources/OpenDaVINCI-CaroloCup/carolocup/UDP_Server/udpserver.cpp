...
void UdpServer::readPendingDatagrams()
{
    while (mUdpSocket->hasPendingDatagrams()) {
        ...
        if (mSerialPort->isOpen()) {
            mSerialPort->writeData(datagram.data(), datagram.size());
        }
...
{   
    ...
    const int rxTimeout = 50;

    while (mSerialPort->bytesAvailable() > 0) {
        QByteArray data = mSerialPort->readAll();
        for(int i = 0;i < data.length();i++) {
            rxData = data[i];
            mSendArray.append((char)rxData);
            ...
            case 1:
                mPayloadLength = rxData;
                mRxState++;
                mRxTimeout = rxTimeout;
                break;

            case 2:
                mRxBuffer[mRxDataPtr++] = rxData;
...
void UdpServer::clearSubscribers()
{
    mSubscribers.clear();
}

void UdpServer::addSubscriber(const QHostAddress &sub)
{
    mSubscribers.append(sub);
}

unsigned short UdpServer::crc16(const unsigned char *buf, unsigned int len)
{
    unsigned int i;
...
void UdpServer::processPacket(const unsigned char *data, int len)
{
    Q_UNUSED(data);
    Q_UNUSED(len);

    for(int i = 0;i < mSubscribers.size();i++) {
        mUdpSocket->writeDatagram(mSendArray, mSubscribers[i], mUdpPort + 1);
    }
    ...
    mUdpSocket->writeDatagram(mSendArray, mLastHostAddress, mUdpPort + 1);
    mSendArray.clear();
}

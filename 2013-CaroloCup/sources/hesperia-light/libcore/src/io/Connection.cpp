...
namespace core {
    namespace io {
        ...
        Connection::Connection(const std::string& ip, const uint32_t port)
                ...
                m_errorHandlerMutex() {
            try {
                m_connection = wrapper::TCPFactory::getInstance().createTCPConnectionTo(ip, port);
            } catch (string& s) {
                HESPERIA_CORE_THROW_EXCEPTION(ConnectException, s);
            }
        ...
        Connection::~Connection() {
            ...
            HESPERIA_CORE_DELETE_POINTER(m_connection);
        }

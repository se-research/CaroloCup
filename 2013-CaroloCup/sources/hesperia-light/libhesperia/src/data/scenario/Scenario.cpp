    ...
    namespace data {
        namespace scenario {
            ...
            ostream& Scenario::operator<<(ostream &out) const {
                ...
                s.write(CRC32 < HESPERIA_CORE_STRINGLITERAL6('h', 'e', 'a', 'd', 'e', 'r') >::RESULT,
                        m_header);

                s.write(CRC32 < HESPERIA_CORE_STRINGLITERAL6('g', 'r', 'o', 'u', 'n', 'd') >::RESULT,
                        m_ground);

                uint32_t numberOfLayers = static_cast<uint32_t>(m_listOfLayers.size());
                s.write(CRC32 < HESPERIA_CORE_STRINGLITERAL8('n', 'u', 'm', 'l', 'a', 'y', 'e', 'r') >::RESULT,
                        numberOfLayers);
                ...
                // Write layers.
                if (numberOfLayers > 0) {
                    s.write(CRC32 < HESPERIA_CORE_STRINGLITERAL6('l', 'a', 'y', 'e', 'r', 's') >::RESULT,
                            sstr.str());
                }
            ...
            istream& Scenario::operator>>(istream &in) {
                ...
                d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL6('h', 'e', 'a', 'd', 'e', 'r') >::RESULT,
                       m_header);

                d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL6('g', 'r', 'o', 'u', 'n', 'd') >::RESULT,
                       m_ground);

                uint32_t numberOfLayers = 0;
                d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL8('n', 'u', 'm', 'l', 'a', 'y', 'e', 'r') >::RESULT,
                       numberOfLayers);

                if (numberOfLayers > 0) {
                    string str;
                    // Read layers into stringstream.
                    d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL6('l', 'a', 'y', 'e', 'r', 's') >::RESULT,
                           str);

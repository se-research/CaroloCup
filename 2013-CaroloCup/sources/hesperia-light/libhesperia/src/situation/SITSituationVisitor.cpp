    ...
    namespace situation {
        ...
        const Header SITSituationVisitor::visitSituationHeader(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from SITUATION expected.");
                }
                header.setName(n->getValue<string>());

                n = node->getNodeFromFirstMatchingChildFor("VERSION");
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from VERSION expected.");
                }
                header.setVersion(n->getValue<string>());

                n = node->getNodeFromFirstMatchingChildFor("DATE");
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from DATE expected.");
                }
                header.setDate(n->getValue<string>());

                n = node->getNodeFromFirstMatchingChildFor("SCENARIO");
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from SCENARIO expected.");
                }
        ...
        Object SITSituationVisitor::visitObject(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from OBJECT expected.");
                }
                o.setName(n->getValue<string>());

                n = node->getNodeFromFirstMatchingChildFor("OBJECTID");
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from OBJECTID expected.");
                }
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from ROTZ expected.");
                }
                o.setRotationZ(n->getValue<double>());

                n = *(node->getChildren("ROTZ").begin() + 1); // BEHAVIOR starts directly after ROTZ.
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from BEHAVIOR expected.");
                }
        ...
        Shape* SITSituationVisitor::visitShape(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from SHAPE expected.");
                }
                string name = n->getValue<string>();

                n = (node->getChildren().at(1))->getFirstChild();
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from SHAPE expected.");
                }
        ...
        const Vertex3 SITSituationVisitor::visitVertex3(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from X expected.");
                }
                v.setX(n->getValue<double>());

                n = node->getNodeFromFirstMatchingChildFor("Y");
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get value from Y expected.");
                }
        ...
        Polygon* SITSituationVisitor::visitPolygon(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from HEIGHT expected.");
                }
                p->setHeight(n->getValue<double>());

                n = *(node->getChildren("COLOR").begin() + 1); // Skip keyword COLOR.
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from COLOR expected.");
                }
        ...
        Rectangle* SITSituationVisitor::visitRectangle(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from LENGTH expected.");
                }
                r->setLength(n->getValue<double>());

                n = node->getNodeFromFirstMatchingChildFor("WIDTH");
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from WIDTH expected.");
                }
        ...
        ComplexModel* SITSituationVisitor::visitComplexModel(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from MODELFILE expected.");
                }
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from POSITION expected.");
                }
                cm->setPosition(visitVertex3(n));

                n = *(node->getChildren("ROTATION").begin() + 1); // Skip keyword POSITION.
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from ROTATION expected.");
                }
                ...
                if (boundingBox.size() != 5) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from BOUNDINGBOX expected.");
                }
...
//                pp.visit(node->getParent()->getLastChild());
        ...
        OnMoving* SITSituationVisitor::visitOnMoving(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from OBJECTID expected.");
                }
        ...
        OnEnteringPolygon* SITSituationVisitor::visitOnEnteringPolygon(ASTNode *node) {
            ...
            if (node != NULL) {
                ...
                if (n == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(SITSituationVisitorException, "Node to get name from VERTEX222 expected.");
                }

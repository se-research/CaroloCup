...
class ScenarioTest : public CxxTest::TestSuite {
    public:
        ...
        void checkData(const Scenario &scn) {
            ...
            const vector<Shape*> &listOfShapes = scn.getGround().getSurroundings().getListOfShapes();
            TS_ASSERT(listOfShapes.size() == 3);

            TS_ASSERT(listOfShapes.at(0)->getType() == Shape::CYLINDER);
            Cylinder *c = dynamic_cast<Cylinder*>(listOfShapes.at(0));
            TS_ASSERT(c->getType() == Shape::CYLINDER);
            ...
            const vector<Vertex3>& listOfVertices = p->getListOfVertices();
            ...
            const vector<Layer> &listOfLayers = scn.getListOfLayers();
            ...
            const vector<Road> &listOfRoads = listOfLayers.at(0).getListOfRoads();
            TS_ASSERT(listOfRoads.size() == 1);
            TS_ASSERT(listOfRoads.at(0).getID() == 1);
            TS_ASSERT(listOfRoads.at(0).getName() == "Road1");

            const vector<Lane> &listOfLanes = listOfRoads.at(0).getListOfLanes();
            ...
            const Lane& lane1 = listOfLanes.at(0);
            const vector<Connector> &listOfConnectorsLane1 = lane1.getLaneModel()->getListOfConnectors();
            ...
            const LaneAttribute& la = lane1.getLaneModel()->getLaneAttribute();
            TS_ASSERT_DELTA(la.getWidth(), 4.5, 1e-3);
            TS_ASSERT(la.getLeftLaneMarking() == LaneAttribute::SOLID_YELLOW);
            TS_ASSERT(la.getRightLaneMarking() == LaneAttribute::SOLID_WHITE);

            const vector<TrafficControl*> listOfTrafficControlsLane1 = lane1.getLaneModel()->getListOfTrafficControls();
            TS_ASSERT(listOfTrafficControlsLane1.size() == 1);

            const TrafficLight *tl = dynamic_cast<TrafficLight*>(listOfTrafficControlsLane1.at(0));
            TS_ASSERT(tl != NULL);
            ...
            const Cylinder *tlc = dynamic_cast<Cylinder*>(tl->getShape());
            TS_ASSERT(tlc != NULL);
            ...
            PointModel *pm = dynamic_cast<PointModel*>(lane1.getLaneModel());
            TS_ASSERT(pm != NULL);

            const vector<IDVertex3> &listOfIdentifiableVerticesLane1 = pm->getListOfIdentifiableVertices();
            ...
            const Lane& lane2 = listOfLanes.at(1);
            StraightLine *sl = dynamic_cast<StraightLine*>(lane2.getLaneModel());
            TS_ASSERT(sl != NULL);

            const vector<TrafficControl*> listOfTrafficControlsLane2 = lane2.getLaneModel()->getListOfTrafficControls();
            ...
            const vector<Vertex3> &tspVertices = tsp->getListOfVertices();
            ...
            const Lane& lane3 = listOfLanes.at(2);
            const vector<Connector> listOfConnectorsLane3 = lane3.getLaneModel()->getListOfConnectors();
            TS_ASSERT(listOfConnectorsLane3.size() == 1);
            TS_ASSERT(listOfConnectorsLane3.at(0).toString() == "(3.3.3.3) -> (1.1.2.1)");

            Clothoid *cl = dynamic_cast<Clothoid*>(lane3.getLaneModel());
            TS_ASSERT(cl != NULL);
            ...
            const Lane& lane4 = listOfLanes.at(3);

...
namespace plugins {
    namespace environmentviewer {
        ...
        EnvironmentViewerGLWidget::~EnvironmentViewerGLWidget() {
            HESPERIA_CORE_DELETE_POINTER(m_root);
            HESPERIA_CORE_DELETE_POINTER(m_selectableNodeDescriptorTree);
        }
        ...
        void EnvironmentViewerGLWidget::createSceneGraph() {
            ...
            if (fin.good()) {
                ...
                if (m_egoStateNode == NULL) {
                    HESPERIA_CORE_THROW_EXCEPTION(InvalidArgumentException, "Could not load car model");
                }
                else {

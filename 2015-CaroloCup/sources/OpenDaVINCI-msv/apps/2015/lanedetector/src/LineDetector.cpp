...
// This is the "body" of line detection
	...
	if (cntDash > 0) {
		...
		for (int i = 0; i < cntDash; i++) {
			...
			if (i != cntDash - 1 && max(dashLines[i + 1].p1.y, dashLines[i + 1].p2.y)
				...
				if (abs(currentDashGoalX - positionX)	< abs(currentDashGoalX - nPositionX)) {
		...
		if (cntDash > 0) {
			...
			if (abs(dashSupPosX - currentDashGoalX) < calcRoadSize * 0.8
				...
				ltu->foundD = true;
				// Set currentDashGoalX --Added
				currentDashGoalX = dashSupPosX;
			}
		}// else {			
		// No new dash line chosen
		// currentDashGoalX = 0;
		// }
	}// else {
	// 	// No new potential dash lines found
	// 	currentDashGoalX = 0;
	// }
	// Conclude whether we got a right line
	if (cntSolid > 0 && !intersectionOn) {
		...
		for (int i = 0; i < cntSolid; i++) {
			if (solidLines[i].slope < 90 && solidLines[i].slope > 0
				...
				// Potential error if there is more than one solid line that satisfy 
				// the if-statement
				ltu->rightLine = solidLines[i];
				ltu->foundR = true;
			}
		}
		// Check if the right line is reasonable
		if (ltu->foundR) {
		...
		// Conclude whether we got a left line
		ltu->leftLine.p1.x = 0;
		ltu->leftLine.p2.x = 0;
		for (int i = 0; i < cntSolid; i++) {
		...
		// Check if the left line is reasonable
		if (ltu->foundL) {
...
LinesToUse* LineDetector::getResult_ltu(){
	return &ltu;
}

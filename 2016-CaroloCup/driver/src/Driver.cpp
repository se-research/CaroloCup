...
// 	cout << "dist_IR_Min" << dist_IR_Min << endl;
	...
	while (getModuleStateAndWaitForRemainingTimeInTimeslice() == coredata::dmcp::ModuleStateMessage::RUNNING) {
		...
		if(sbd.containsKey_MapOfDistances(3))
            dist_IR_SRF=sbd.getValueForKey_MapOfDistances(3);
    
		dist_IR_RL=-2;
		if(sbd.containsKey_MapOfDistances(0))
            dist_IR_SRF=sbd.getValueForKey_MapOfDistances(0);
    
		dist_IR_RR=-2;
		if(sbd.containsKey_MapOfDistances(1))
            dist_IR_SRF=sbd.getValueForKey_MapOfDistances(1);
    
		dist_IR_SR=-2;
		if(sbd.containsKey_MapOfDistances(2))
            dist_IR_SRF=sbd.getValueForKey_MapOfDistances(2);
    
		dist_US_Front=-2;
		if(sbd.containsKey_MapOfDistances(4))
            dist_IR_SRF=sbd.getValueForKey_MapOfDistances(4);
    
		dist_US_Rear=-2;
		if(sbd.containsKey_MapOfDistances(5))
            dist_IR_SRF=sbd.getValueForKey_MapOfDistances(5);
    
		totalDistance=-2;
		if(sbd.containsKey_MapOfDistances(6))
            dist_IR_SRF=sbd.getValueForKey_MapOfDistances(6);
        /*

...
// Constructor
  obstacleDriver::obstacleDriver (const int32_t &argc, char **argv) :
      TimeTriggeredConferenceClientModule (argc, argv, "Driver"), m_hasReceivedLaneDetectionData (
	...
	if (first)
	  {
		...
		int urF=-2;
		if(sbd.containsKey_MapOfDistances(4))
            urF=sbd.getValueForKey_MapOfDistances(4);
    
		int irFR_side=-2;
		if(sbd.containsKey_MapOfDistances(3))
            irFR_side=sbd.getValueForKey_MapOfDistances(3);
    
		int irRR_side=-2;
		if(sbd.containsKey_MapOfDistances(2))
            irRR_side=sbd.getValueForKey_MapOfDistances(2);
    
		int irRR=-2;
		if(sbd.containsKey_MapOfDistances(1))
            irRR=sbd.getValueForKey_MapOfDistances(1);
...
/*	//Check for intersection
	...
	vc.setFlashingLightsLeft(leftBlink);
	vc.setFlashingLightsRight(rightBlink);
  ...
  obstacleDriver::overtaking (SensorBoardData sensorData)
  {
    ...
    //int irR = sensorData.getDistance(1);
		...
		if(sensorData.containsKey_MapOfDistances(4))
            urF=sensorData.getValueForKey_MapOfDistances(4);
    
		int irFR_side=-2;
		if(sensorData.containsKey_MapOfDistances(3))
            irFR_side=sensorData.getValueForKey_MapOfDistances(3);
    
		int irRR_side=-2;
		if(sensorData.containsKey_MapOfDistances(2))
            irRR_side=sensorData.getValueForKey_MapOfDistances(2);
    
		int irR=-2;
		if(sensorData.containsKey_MapOfDistances(1))
            irR=sensorData.getValueForKey_MapOfDistances(1);

...
void LineDetector::filterAndMerge() {
    for (int j = 0; j < cntSolid; j++) {
        ...
        Point center;
        center=getLineCenter(solidLines[j].p1,solidLines[j].p2);
        ...
        if ((solidLines[j].slope > MIN_ANGLE - 5
            ...
            for (int l = 0; l < cntDash; l++) {
                Point dashCenter;
                dashCenter=getLineCenter(dashLines[l].p1,dashLines[l].p2);
            ...
            for (int k = j + 1; k < cntSolid; k++) {
                Point sldCenter;
                sldCenter=getLineCenter(solidLines[k].p1,solidLines[k].p2);
    ...
    if (cntDash > 0) {
        ...
        for (int i = 0; i < cntDash; i++) {
            if (dashLines[i].p1.y < dashLines[i].p2.y) {
                ...
                //WE COULD HAVE A FUNCTION TO SWAP THE POINTS

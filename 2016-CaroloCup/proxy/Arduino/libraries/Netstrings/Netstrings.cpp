...
String decodedNetstring(String netstring){
	if (netstring.length() < 3) return "error1"; //if the Netstring is less than 3 characters, it's either an invalid one or contains an empty string
	...
	if (controlDigits < 1) return "error3"; //if the control digit is smaller than 1, then it's either not a digit or the Netstring is invalid
	String command = netstring.substring(semicolonIndex+1); //parse after the semicolon until the end of the string
	if (!command.length()) return "error4"; // if it's an empty string, return "error"
	if (command.substring(command.length() -1) == ",") command.remove(command.length()-1); //if last character is a comma, remove it
//	if (command.length() != controlDigits) return "error5"; //if string's length isn't equal with the control digits, it's an invalid Netstring
	return command;
}

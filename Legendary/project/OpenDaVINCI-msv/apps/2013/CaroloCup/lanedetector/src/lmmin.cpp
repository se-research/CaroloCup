            ...
            const lm_control_struct *C, lm_status_struct *S )
{
    ...
    for( outer=0; ; ++outer )
    {
        ...
        do
        {
            ...
            ratio = !fEqual(prered, 0) ? actred/prered : 0;
              ...
              double *sdiag, double *aux, double *xdi)
{
    ...
    *par = MIN(*par, paru);
    ...
    for (iter=0; ; iter++)
    {
        ...
        if ((fabs(fp) <= p1 * delta)
                || ((fEqual(parl , 0.)) && (fp <= fp_old) && (fp_old < 0.))
                || (iter == 10))
        {
#ifdef LMFIT_DEBUG_MESSAGES

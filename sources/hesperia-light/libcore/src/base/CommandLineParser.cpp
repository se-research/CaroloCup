    ...
    namespace base {
        ...
        const CommandLineArgument CommandLineParser::getCommandLineArgument(const string &argument) throw (CommandLineParserException) {
            ...
            if (it == m_listOfArgumentsToParse.end()) {
                errno = 0;
                HESPERIA_CORE_THROW_EXCEPTION(CommandLineParserException, "Argument " + argument + " unknown to command line parser.");
            }
        ...
        void CommandLineParser::parse(const int32_t &argc, char **argv) {
            ...
            for (int32_t i = 1; i < argc; i++) {
                ...
                if (!handled) {
                    HESPERIA_CORE_THROW_EXCEPTION(CommandLineParserException, "Unknown command line argument '" + cmdLineValue + "'.");
                }
            }
        }
    }

...
namespace core {
    namespace base {
        ...
        QueryableNetstringsDeserializer::QueryableNetstringsDeserializer(istream &in) :
            ...
            // This MUST be a blank (Win32 suc*s)!
            m_buffer << " ";
        ...
        /* TODO:
        In QueryableNetstringsDeserializer.h:
        template<typename T>
        inline
        T read(const uint32_t &id) {
            map<uint32_t, streampos>::iterator it = m_values.find(id);

            T t;
            if (it != m_values.end()) {
                m_buffer.seekg(it->second);
                m_buffer.read(reinterpret_cast<char *>(&t), sizeof(T));
            }
            return t;
        }
        */
        ...
        void QueryableNetstringsDeserializer::read(const uint32_t id, string &s) {
            ...
            if (it != m_values.end()) {
                ...
                HESPERIA_CORE_DELETE_ARRAY(str);
            }
        }

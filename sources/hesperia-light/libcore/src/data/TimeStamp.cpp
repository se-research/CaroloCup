...
namespace core {
    namespace data {
        ...
        uint32_t TimeStamp::getMinute() const {
            ...
            const int32_t secondsSinceMidnight = seconds - daysSince01011970 * (60*60*24);
            const int32_t hour = secondsSinceMidnight / (60*60);
            const int32_t minute = ((double)secondsSinceMidnight / (60.0*60.0) - hour) * 60;
            return minute;
        }
        ...
        ostream& TimeStamp::operator<<(ostream &out) const {
            ...
            s.write(CRC32 < HESPERIA_CORE_STRINGLITERAL3('s', 'e', 'c') >::RESULT,
                    m_seconds);

            s.write(CRC32 < HESPERIA_CORE_STRINGLITERAL3('m', 'i', 'c') >::RESULT,
                    m_microseconds);
        ...
        istream& TimeStamp::operator>>(istream &in) {
            ...
            d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL3('s', 'e', 'c') >::RESULT,
                   m_seconds);

            d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL3('m', 'i', 'c') >::RESULT,
                   m_microseconds);

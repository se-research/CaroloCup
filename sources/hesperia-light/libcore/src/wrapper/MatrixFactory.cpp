...
namespace core {
    namespace wrapper {
        ...
        Mutex* MatrixFactory::m_singletonMutex = MutexFactory::getInstance().createMutex();
        ...
        MatrixFactory& MatrixFactory::getInstance() {
            MatrixFactory::m_singletonMutex->lock();
            {
                if (MatrixFactory::m_singleton == NULL) {
                    ...
                    DisposalService::getInstance().addDisposableForFinalRemoval(MatrixFactory::m_singleton);
                }
            }

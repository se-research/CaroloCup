...
namespace hesperia {
    namespace base {
        ...
        ConferenceClientModule::ConferenceClientModule(const int32_t &argc, char **argv, const string &name) throw (InvalidArgumentException, NoDatabaseAvailableException) :
            ...
            m_keyValueDataStore = SharedPointer<KeyValueDataStore>(new KeyValueDataStore(wrapper::KeyValueDatabaseFactory::getInstance().createKeyValueDatabase("")));

            // Create conference.
            m_conference = ContainerConferenceFactory::getInstance().getContainerConference(getMultiCastGroup());
            if (m_conference == NULL) {
                HESPERIA_CORE_THROW_EXCEPTION(InvalidArgumentException, "ContainerConference invalid!");
            }
        ...
        ConferenceClientModule::~ConferenceClientModule() {
            ...
            HESPERIA_CORE_DELETE_POINTER(m_conference);

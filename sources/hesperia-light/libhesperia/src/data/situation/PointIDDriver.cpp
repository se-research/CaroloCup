    ...
    namespace data {
        namespace situation {
            ...
            void PointIDDriver::cleanUp() {
                HESPERIA_CORE_DELETE_POINTER(m_startType);
                HESPERIA_CORE_DELETE_POINTER(m_stopType);
            }
            ...
            istream& PointIDDriver::operator>>(istream &in) {
                ...
                d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL8('h', 'a', 's', 's', 't', 'a', 'r', 't') >::RESULT,
                       hasStartType);
                ...
                d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL7('h', 'a', 's', 's', 't', 'o', 'p') >::RESULT,
                       hasStopType);

                if (hasStopType) {
                    string str;
                    // Read shapes into stringstream.
                    d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL4('s', 't', 'o', 'p') >::RESULT,
                           str);
                ...
                d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL1('a') >::RESULT,
                       m_constantAcceleration);

                d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL1('v') >::RESULT,
                       m_constantVelocity);

                uint32_t numberOfPointIDs = 0;
                d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL7('n', 'u', 'm', 'p', 'i', 'd', 's') >::RESULT,
                       numberOfPointIDs);

                if (numberOfPointIDs > 0) {
                    string str;
                    // Read identifiable vertices into stringstream.
                    d.read(CRC32 < HESPERIA_CORE_STRINGLITERAL4('p', 'i', 'd', 's') >::RESULT,
                           str);

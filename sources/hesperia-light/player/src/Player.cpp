    ...
    ModuleState::MODULE_EXITCODE Player::body() {
        ...
        while (getModuleState() == ModuleState::RUNNING) {
            if (playing) {
                ...
                // Here, the "actual" container is send while discarding player commands.
                // TODO: Filter data.
                if ( (actual.getDataType() != Container::UNDEFINEDDATA) &&
            ...
            if (remoteControl) {
                if (!(m_playerControl.isEmpty())) {
                    ...
                    pc = container.getData<hesperia::data::player::PlayerCommand>();

                    cerr << "Player: " << pc.toString() << endl;
                    switch (pc.getCommand()) {
                    case hesperia::data::player::PlayerCommand::PLAY:
                        playing = true;
                        break;
                    case hesperia::data::player::PlayerCommand::PAUSE:
                        playing = false;
                        break;
                    case hesperia::data::player::PlayerCommand::STEP_FORWARD:
                        playing = true;
                        doStep = true;
                        break;
                    case hesperia::data::player::PlayerCommand::REWIND:

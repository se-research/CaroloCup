...
namespace supercomponent {
    ...
    SuperComponent::SuperComponent(const int &argc, char **argv) :
        ...
        if (configStream.good()) {
            ...
            HESPERIA_CORE_THROW_EXCEPTION(ConfigurationFileNotFoundException, "Configuration stream invalid.");
        }

        ServerInformation serverInformation("0.0.0.0",
                                            CONNECTIONSERVER_PORT_BASE + getCID());
    ...
    void SuperComponent::handleConnectionLost(const ModuleDescriptor& md) {
        // Behandelt den Fall, dass eine Verbindung abgebrochen ist und wir
        // nicht über eine Beeindigung des Modules informiert wurden.
